...
<template>
  <div class="w-full">
    ...
    <div class="w-3/4 mx-auto mb-6 flex">
      <button class="w-fit rounded border hover:border-slate-50 px-1.5 me-1 btn-shadow" @click="handleClear">C</button>
      <input type="text" name="" id="" class="w-3/4 px-2 text-slate-600 rounded outline-1 outline-slate-400 bg-slate-50 btn-shadow" readonly :value="password" />
      <button class="ms-1" @click="handleSubmit"><i class="ri-arrow-right-up-line p-1 bg-slate-400 hover:bg-slate-600 rounded btn-shadow"></i></button>
    </div>
  </div>
</template>
<style scoped>
